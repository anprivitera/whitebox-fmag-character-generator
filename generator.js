...
function generateCharacterDetails(
  generatedAttributes,
  generatedCharacterClass,
  generatedCharacterRace,
  armorClassPreference
) {
  function determineXPBonus() {
    let xpBonus = 0;
    if (generatedAttributes[4].attributeValue >= 15) {
      xpBonus += 5;
    }
    if (generatedAttributes[5].attributeValue >= 15) {
      xpBonus += 5;
    }
  ...
  let characterHP =
    generatedCharacterClass.HDatLevel1 + generatedAttributes[2].modifierValue;
  if (characterHP <= 0) {
    characterHP = 1;
  }

  const toHitMelee = `${
    generatedCharacterClass.toHitAtLevel1 +
      generatedAttributes[0].modifierValue >
    0
      ? "+"
      : ""
  }${
    generatedCharacterClass.toHitAtLevel1 + generatedAttributes[0].modifierValue
  }`;
  const toHitMissile = `${
    generatedCharacterClass.toHitAtLevel1 +
    ...
    generatedCharacterClass.toHitAtLevel1 + generatedAttributes[1].modifierValue
  }`;

  let maxHirelings = null;
  let hirelingsLoyalty = null;
  if (generatedAttributes[5].attributeValue <= 4) {
    maxHirelings = 1;
    hirelingsLoyalty = -2;
  } else if (
    generatedAttributes[5].attributeValue >= 5 &&
    generatedAttributes[5].attributeValue <= 6
  ) {
    maxHirelings = 2;
    hirelingsLoyalty = -2;
  } else if (
    generatedAttributes[5].attributeValue >= 7 &&
    generatedAttributes[5].attributeValue <= 8
  ) {
    maxHirelings = 3;
    hirelingsLoyalty = -1;
  } else if (
    generatedAttributes[5].attributeValue >= 9 &&
    generatedAttributes[5].attributeValue <= 12
  ) {
    maxHirelings = 4;
    hirelingsLoyalty = 0;
  } else if (
    generatedAttributes[5].attributeValue >= 13 &&
    generatedAttributes[5].attributeValue <= 15
  ) {
    maxHirelings = 5;
    hirelingsLoyalty = 1;
  } else if (
    generatedAttributes[5].attributeValue >= 16 &&
    generatedAttributes[5].attributeValue <= 17
  ) {
    maxHirelings = 6;
    hirelingsLoyalty = 2;
  } else {
  ...
  let descendingArmorClass = 9 - generatedAttributes[1].modifierValue;
  let ascendingArmorClass = 10 + generatedAttributes[1].modifierValue;
  let gearWeight = 10;
...
// refresher.addEventListener("click", rollAttributes());
...
refresher.addEventListener(
  ...
  generateCharacter(
    rollAttributes,
    document.getElementById("armor-class").value
  )
);
